--[[
	...
	SMUI Interface Suite
	RayField fork by Weh, SpotyMine9022, SpotyMine9022A
...
local SMUIFolder = "SMUI"
local ConfigurationFolder = SMUIFolder.."/Configurations"
local ConfigurationExtension = ".smui"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		SMUIOpen = {Type = 'bind', Value = 'K', Name = 'SMUI Keybind'},
		-- buildwarnings
		-- SMUIprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.SMUIOpen"] = "J"
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(SMUIFolder) then
				if isfile and isfile(SMUIFolder..'/settings'..ConfigurationExtension) then
					file = readfile(SMUIFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"SMUIOpen":{"Value":"K","Type":"bind","Name":"SMUI Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"SMUI Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('SMUI had an issue accessing configuration saving capability.')
		end
	end
end
...
local SMUI = useStudio and script.Parent:FindFirstChild('SMUI') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local SMUIDestroyed = false -- True when SMUILibrary:Destroy() is called

repeat
	if SMUI:FindFirstChild('Build') and SMUI.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('SMUI | Build Mismatch')
		print('SMUI may encounter issues as you are running an incompatible interface version ('.. ((SMUI:FindFirstChild('Build') and SMUI.Build.Value) or 'No Build') ..').\n\nThis version of SMUI is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, SMUI = SMUI, useStudio and script.Parent:FindFirstChild('SMUI') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(SMUI)
	SMUI.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	SMUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == SMUI.Name and Interface ~= SMUI then
			Interface.Enabled = false
			Interface.Name = "SMUI-Old"
		end
	end
elseif not useStudio then
...
if SMUI.AbsoluteSize.X < minSize.X and SMUI.AbsoluteSize.Y < minSize.Y then
...
local Main = SMUI.Main
local MPrompt = SMUI:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = SMUI:FindFirstChild('Drag')
...
SMUI.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/SMUI/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = SMUI.Notifications

local SelectedTheme = SMUILibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = SMUILibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	SMUI.Main.BackgroundColor3 = SelectedTheme.Background
	SMUI.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	SMUI.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	SMUI.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	SMUI.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	SMUI.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	SMUI.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		SMUI.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		SMUI.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(SMUI:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("SMUI | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("SMUI | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('SMUI had an issue decoding the configuration file, please try delete the file and reopen SMUI.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(SMUILibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("SMUI | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--SMUILibrary:Notify({Title = "SMUI Flags", Content = "SMUI was unable to find '"..FlagName.. "' in the save file.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(SMUILibrary.Flags) do
...
function SMUILibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('SMUI | Not seeing your actions in notifications?')
		end
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			SMUILibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			SMUILibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "SMUIOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		if writefile then
			writefile(SMUIFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('SMUI Settings', 0, true)

	if TabList['SMUI Settings'] then
		TabList['SMUI Settings'].LayoutOrder = 1000
	end

	if Elements['SMUI Settings'] then
		Elements['SMUI Settings'].LayoutOrder = 1000
	end
...
function SMUILibrary:CreateWindow(Settings)
	print('creating window')
	if SMUI:FindFirstChild('Loading') then
		if getgenv and not getgenv().SMUICached then
			SMUI.Enabled = true
			SMUI.Loading.Visible = true

			task.wait(1.4)
			SMUI.Loading.Visible = false
		end
	end

	if getgenv then getgenv().SMUICached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				SMUILibrary:Notify({Title = 'Build Mismatch', Content = 'SMUI may encounter issues as you are running an incompatible interface version ('.. ((SMUI:FindFirstChild('Build') and SMUI.Build.Value) or 'No Build') ..').\n\nThis version of SMUI is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "SMUI"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "SMUI Interface Suite" then
		LoadingFrame.Version.Text = "SMUI UI"
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(SMUIFolder.."/Key System") then
			makefolder(SMUIFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("SMUI | "..Key.." Error " ..tostring(Response))
					warn('')
				end
			end
		end
		...
		if isfile and isfile(SMUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(SMUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(SMUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						SMUILibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called SMUILibrary:Destroy()
				if SMUIDestroyed then
					return
				end
				if not Success then
					...
					print("SMUI | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= SMUILibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					SMUILibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			SMUI.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("SMUI | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					SMUILibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("SMUI | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					SMUILibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("SMUI | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					SMUILibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= SMUILibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("SMUI | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						SMUILibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("SMUI | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					SMUILibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			SMUILibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			SMUILibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('SMUI had an issue creating settings.') end
...
function SMUILibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function SMUILibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function SMUILibrary:Destroy()
	SMUIDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['SMUI Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "SMUIOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function SMUILibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				SMUILibrary:Notify({Title = "SMUI Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			SMUILibrary:Notify({Title = "SMUI Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('SMUI Configurations Error | '..tostring(result))
			SMUILibrary:Notify({Title = "SMUI Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
